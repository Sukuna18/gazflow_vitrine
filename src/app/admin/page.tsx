...
export default async function AdminPage() {
  ...
  const [products, orders, zones, settings, admins] = await Promise.all([
    ...
    prisma.deliveryZone.findMany({ orderBy: { id: "desc" } }),
    prisma.siteSettings.upsert({ where: { id: 1 }, update: {}, create: { id: 1 } }),
    prisma.admin.findMany({ select: { id: true, name: true, email: true, active: true, createdAt: true }, orderBy: { createdAt: "desc" } }),
  ]);
