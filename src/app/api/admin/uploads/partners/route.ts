...
const imageSchema = z
  .instanceof(File)
  .refine((file) => extensions.has(file.type), "Formats acceptes : JPG, PNG, WEBP ou SVG.")
...
export async function POST(request: Request) {
  ...
  const uploadDir = process.env.PARTNERS_UPLOAD_DIR ?? path.join(process.cwd(), "public", "uploads", "partners");
  const publicPath = process.env.PARTNERS_PUBLIC_UPLOAD_PATH ?? "/uploads/partners";
